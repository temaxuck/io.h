...
#define T_BUFFER_DUP(b) ({                              \
            IO_Buffer d = T_BUFFER_EMPTY((b)->cap);     \
...
#define T_BUFFER_ASSERT_FOR_BUFFER(expr, b) ({              \
            bool result = T_BUFFER_ASSERT(expr);            \
            if (!result) printf("%s\n", t_buffer_repr(b));  \
            result;                                         \
        })
...
void t_buffer_case_init_empty(void) {
    IO_Buffer before = {0};
    T_BUFFER_ASSERT(io_buffer_init(&before, 6) == IO_ERR_OK);
    T_BUFFER_ASSERT_FOR_BUFFER(before.end == before.start && before.start == before.buf, &before);

    IO_Buffer after  = T_BUFFER_DUP(&before);
    size_t len = io_buffer_len(&after);
    T_BUFFER_ASSERT_FOR_BUFFER(len == 0, &after);
...
void t_buffer_case_append_exactly_capacity_no_wrap(void) {
    IO_Buffer b = T_BUFFER_EMPTY(6);
    T_BUFFER_ASSERT_FOR_BUFFER(io_buffer_append(&b, "123456", 6) == IO_ERR_OK, &b);
    size_t len = io_buffer_len(&b);
    T_BUFFER_ASSERT_FOR_BUFFER(len == 6, &b);
    T_BUFFER_ASSERT_FOR_BUFFER(strncmp(b.buf, "123456", len) == 0, &b);

    io_buffer_free(&b);
}

void t_buffer_case_append_beyond_capacity(void) {
    IO_Buffer before = T_BUFFER_EMPTY(6);
    IO_Buffer after  = T_BUFFER_DUP(&before);
    T_BUFFER_ASSERT_FOR_BUFFER(io_buffer_append(&after, "1234567", 7) == IO_ERR_OOB, &after);
    T_BUFFER_ASSERT(t_buffer_cmp(&after, &before) == true);
...
void t_buffer_case_append_zero_bytes(void) {
    IO_Buffer before = T_BUFFER_EMPTY(6);
    IO_Buffer after  = T_BUFFER_DUP(&before);
    T_BUFFER_ASSERT_FOR_BUFFER(io_buffer_append(&after, "", 0) == IO_ERR_OK, &after);
    T_BUFFER_ASSERT(t_buffer_cmp(&after, &before) == true);
...
void t_buffer_case_nspit_contiguous_read_no_wrap(void) {
    IO_Buffer before = T_BUFFER_EMPTY(6);
    T_BUFFER_ASSERT_FOR_BUFFER(io_buffer_append(&before, "123456", 6) == IO_ERR_OK, &before);

    IO_Buffer after = T_BUFFER_DUP(&before);
    char dest[6] = {0};
    T_BUFFER_ASSERT(io_buffer_nspit(&after, dest, 6) == IO_ERR_OK);
    T_BUFFER_ASSERT(strncmp(dest, "123456", 6) == 0);
...
void t_buffer_case_nspit_partial_contiguous_read(void) {
    ...
    IO_Buffer after = T_BUFFER_DUP(&before);
    char dest[3] = {0};
    T_BUFFER_ASSERT(io_buffer_nspit(&after, dest, 3) == IO_ERR_OK);
    T_BUFFER_ASSERT(strncmp(dest, "123", 3) == 0);
...
void t_buffer_case_wrapped_buffer_read_nspit_across_wrap(void) {
    ...
    T_BUFFER_ASSERT_FOR_BUFFER(len == 4, &before);

    IO_Buffer after = T_BUFFER_DUP(&before);
    char dest[4] = {0};
    T_BUFFER_ASSERT(io_buffer_nspit(&after, dest, 4) == IO_ERR_OK);
...
void t_buffer_case_append_when_buffer_is_wrapped_fills_remaining_capacity(void) {
    ...
    T_BUFFER_ASSERT(io_buffer_append(&after, "EF", 2) == IO_ERR_OK);
    T_BUFFER_ASSERT(io_buffer_len(&after) == 6);
    T_BUFFER_ASSERT(after.start - after.buf == before.start - before.buf);
    T_BUFFER_ASSERT(after.end == after.buf + 3);
...
void t_buffer_case_nspit_requesting_more_bytes_than_available(void) {
    ...
    char dest[7] = {0};
    T_BUFFER_ASSERT(io_buffer_nspit(&b, dest, 7) == IO_ERR_OOB);
    if (!T_BUFFER_ASSERT(strcmp(dest, "") == 0)) printf("ERROR: dest=%.*s (expected: (null))\n", 4, dest);
...
void t_buffer_case_nspit_reading_across_the_wrap_boundary_partial(void) {
    ...
    char dest[3] = {0};
    T_BUFFER_ASSERT(io_buffer_nspit(&after, dest, 3) == IO_ERR_OK);
    if (!T_BUFFER_ASSERT(strncmp(dest, "ABC", 3) == 0)) printf("ERROR: dest=%.*s (expected: ABC)\n", 3, dest);
...
void t_buffer_case_minimal_capacity_cap_eq_1(void) {
    IO_Buffer before = {0};
    T_BUFFER_ASSERT(io_buffer_init(&before, 1) == IO_ERR_OK);
    T_BUFFER_ASSERT_FOR_BUFFER(before.end == before.start && before.start == before.buf, &before);

    T_BUFFER_ASSERT(io_buffer_append(&before, "A", 1) == IO_ERR_OK);
    T_BUFFER_ASSERT(io_buffer_len(&before) == 1);
    T_BUFFER_ASSERT_FOR_BUFFER(before.buf[0] == 'A', &before);
    T_BUFFER_ASSERT_FOR_BUFFER(before.start == before.buf, &before);
    T_BUFFER_ASSERT_FOR_BUFFER(before.end == before.buf + 1, &before);

    char dest[1] = {0};
    T_BUFFER_ASSERT(io_buffer_nspit(&before, dest, 1) == IO_ERR_OK);
    if (!T_BUFFER_ASSERT(strncmp(dest, "A", 1) == 0)) printf("ERROR: dest=%.*s (expected: ABC)\n", 1, dest);

    IO_Buffer after  = T_BUFFER_DUP(&before);
    T_BUFFER_ASSERT(io_buffer_append(&after, "B", 1) == IO_ERR_OOB);
    T_BUFFER_ASSERT(t_buffer_cmp(&after, &before) == true);
...
void t_buffer_case_nspit_with_n_eq_0(void) {
    ...
    T_BUFFER_ASSERT(io_buffer_nspit(&after, dest, 0) == IO_ERR_OK);
...
void t_buffer_case_nadvance_contiguous_data(void) {
    ...
    T_BUFFER_ASSERT(io_buffer_nadvance(&b, 4) == 4);
    T_BUFFER_ASSERT_FOR_BUFFER(b.start == b.buf + 4, &b);
    T_BUFFER_ASSERT_FOR_BUFFER(b.end == b.buf + 6, &b);
...
void t_buffer_case_nadvance_wrap(void) {
    ...
    T_BUFFER_ASSERT(io_buffer_nadvance(&b, 3) == 3);
    T_BUFFER_ASSERT_FOR_BUFFER(b.start == b.buf + 1, &b);
    T_BUFFER_ASSERT_FOR_BUFFER(b.end == b.buf + 2, &b);
...
void t_buffer_case_nadvance_to_0(void) {
    ...
    T_BUFFER_ASSERT(io_buffer_nadvance(&b, 0) == 0);
    T_BUFFER_ASSERT_FOR_BUFFER(b.start == b.buf, &b);
    T_BUFFER_ASSERT_FOR_BUFFER(b.end == b.buf + 6, &b);
...
void t_buffer_case_nadvance_with_empty_buffer(void) {
    ...
    T_BUFFER_ASSERT(io_buffer_nadvance(&b, 4) == 0);
    T_BUFFER_ASSERT_FOR_BUFFER(b.start == b.buf && b.end == b.buf, &b);
...
void t_buffer_case_nadvance_beyond_capacity(void) {
    ...
    T_BUFFER_ASSERT(io_buffer_nadvance(&b, 69) == 6);
    T_BUFFER_ASSERT_FOR_BUFFER(b.start == b.end && b.end == b.buf + 6, &b);
...
#undef XX
      ...
      printf("INFO: PASSED: %ld; FAILED: %ld.\n", T_BUFFER_PASSED, T_BUFFER_FAILED);
}
